...
async def debug_providers():
    """Debug endpoint: test each LLM provider directly."""
        ...
        try:
            async with _httpx.AsyncClient() as client:
                r = await client.post(
                    ...
                    json={
                        "contents": [{"parts": [{"text": "Say hello in JSON"}]}],
                        "generationConfig": {"temperature": 0.1, "maxOutputTokens": 100, "responseMimeType": "application/json"}
                    },
                    timeout=30,
                )
                results["gemini"] = {"status": r.status_code, "body": r.text[:500]}
        except Exception as e:
            results["gemini"] = {"error": str(e)}
    else:
        results["gemini"] = {"error": "no GOOGLE_API_KEY"}
    ...
    if groq_key:
        try:
            async with _httpx.AsyncClient() as client:
                ...
                results["groq"] = {"status": r.status_code, "body": r.text[:500]}
        except Exception as e:
            results["groq"] = {"error": str(e)}
    else:
        results["groq"] = {"error": "no GROQ_API_KEY"}
    ...
    if or_key:
        try:
            async with _httpx.AsyncClient() as client:
                ...
                results["openrouter"] = {"status": r.status_code, "body": r.text[:500]}
        except Exception as e:
            results["openrouter"] = {"error": str(e)}
    else:
        results["openrouter"] = {"error": "no OPENROUTER_API_KEY"}
