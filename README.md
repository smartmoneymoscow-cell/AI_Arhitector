# Architect v10.4.0 — AI Architecture Generator

Генерация 3D-моделей зданий и интерьеров по текстовому описанию на русском языке.

## Что нового в v10.4.0

- **3D-модель снова работает** — исправлен критический баг: pipeline прерывался после вопросов и не доходил до генерации
- **Канвас resize** — исправлен рендеринг Three.js (0×0 → корректные размеры)
- **HTML рендеринг** — reasoning cards, decomposition trees, кнопки вопросов теперь отображаются корректно
- **JS syntax fix** — исправлены неэкранированные кавычки в onmouseover/onmouseout
- **Mobile hamburger menu** — добавлена кнопка открытия чата на мобильных
- **Tablet chat panel** — чат-панель по умолчанию открыта на планшетах
- **Auto-scroll** — плавная прокрутка чата к новым сообщениям
- **CV-тестирование** — документация методологии визуального анализа скриншотов
