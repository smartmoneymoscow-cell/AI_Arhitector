...
def add_sofa(location, color="#4a4a4a", width=2.0):
    ...
    for side in [-1, 1]:
        bpy.ops.mesh.primitive_cube_add(size=1, 
            location=(location[0] + side * (width/2 - 0.1), location[1], location[2]+0.2))
...
def add_table(location, width=1.2, height=0.75, color="#8b6914"):
    ...
    for dx, dy in [(-1,-1),(1,-1),(-1,1),(1,1)]:
        bpy.ops.mesh.primitive_cube_add(size=1,
            location=(location[0]+dx*(width/2-0.08), location[1]+dy*(width*0.6/2-0.08), location[2]+height/2))
...
def add_bed(location, width=1.8, length=2.0, color="#f0f0f0"):
    ...
    for side in [-1, 1]:
        bpy.ops.mesh.primitive_cube_add(size=1,
            location=(location[0]+side*0.3, location[1]+length/2-0.3, location[2]+0.55))
...
def temp_to_rgb(kelvin):
    ...
    b = 1.0 if temp >= 66 else (max(0, min(1, (138.5177312231 * math.log(temp - 10) - 305.0447927307) / 255)) if temp > 19 else 0)
    return (r, g, b)
...
def main():
    ...
    for item in furniture_list:
        if item == "sofa":
            objects.extend(add_sofa((0, -room_l/2 + 1, 0), color=mats["accent_color"] if "accent_color" in style else "#4a4a4a"))
        elif item == "table":
    ...
    if cam_pos == "corner":
        setup_interior_camera(
            (room_w/2 - 0.5, -room_l/2 + 0.5, room_h * 0.7),
            (0, 0, room_h * 0.4),
            focal_length=24
        )
    elif cam_pos == "center":
