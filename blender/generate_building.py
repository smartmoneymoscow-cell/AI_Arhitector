...
def generate_building(params):
    ...
    # ── Foundation ──
    foundation = create_box("Foundation", (0, 0, -0.15), (W + 0.6, L + 0.6, 0.3),
                           create_material("Concrete", "#808080", 0.95, 0.0))
    ...
    for floor in range(floors):
        ...
        for i in range(n_win):
            ...
            try:
                cut_window(bpy.data.objects.get(f"Wall_F_{floor}") or all_objects[-4],
                          pos, win_w, win_h, glass_mat)
            except:
        ...
        if params.get("has_balcony") and floor > 0:
            balc = create_box(f"Balcony_{floor}",
                            (0, -L/2 - 1.5, z_base + 0.05),
                            (3, 1.5, 0.1), floor_mat)
            # Railing
            rail = create_box(f"Railing_{floor}",
                            (0, -L/2 - 2.2, z_base + 0.6),
                            (3, 0.05, 1.0), facade_mat)
...
def main():
    ...
    # Generate
    print(f"[ArchAI] Generating {params['type']}: {params['floors']} floors, "
