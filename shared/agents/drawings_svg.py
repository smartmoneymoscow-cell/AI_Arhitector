...
def generate_floor_plan_svg(params: dict, rooms: list = None) -> str:
    ...
    if rooms:
        ...
        for room in rooms:
            ...
            lines.append(
                f'<line x1="{rx - rw // 2}" y1="{ry + rd // 2 - 10}" x2="{rx - rw // 2 + 15}" y2="{ry + rd // 2 - 10}" stroke="#ffffff" stroke-width="3"/>'
            )
    ...
    lines.append(
        f'<text x="{dim_x + 6}" y="{(y1 + y2) // 2}" class="dim" transform="rotate(90,{dim_x + 6},{(y1 + y2) // 2})">{length} м</text>'
    )
    ...
    lines.append(
        f'<text x="{margin}" y="{svg_h - 25}" class="subtitle">{width}×{length} м | Масштаб 1:{int(1000 / scale) if scale else 60}</text>'
    )
...
def generate_section_svg(params: dict) -> str:
    ...
    lines.append(
        f'<rect x="{x1 - 10}" y="{fnd_y}" width="{int(width * scale) + 20}" height="{int(fnd_h)}" class="foundation"/>'
    )
    # Hatch pattern
    ...
    for fl in range(floors):
        ...
        # Left wall
        lines.append(
            f'<rect x="{x1}" y="{y_bottom}" width="{int(wall_thick)}" height="{int(fH * scale)}" class="wall-section"/>'
        )
        # Right wall
        lines.append(
            f'<rect x="{x2 - int(wall_thick)}" y="{y_bottom}" width="{int(wall_thick)}" height="{int(fH * scale)}" class="wall-section"/>'
        )
        ...
        if fl > 0:
            slab_h = 0.2 * scale
            lines.append(
                f'<rect x="{x1}" y="{y_top - int(slab_h)}" width="{int(width * scale)}" height="{int(slab_h)}" class="floor-section"/>'
            )

        # Room label
        lines.append(
            f'<text x="{ox}" y="{(y_bottom + y_top) // 2}" class="label" text-anchor="middle">Этаж {fl + 1}</text>'
        )
        ...
        lines.append(
            f'<rect x="{x1}" y="{win_y}" width="{int(wall_thick)}" height="{win_h}" fill="#bfe3f5" stroke="#0e7490"/>'
        )
        lines.append(
            f'<rect x="{x2 - int(wall_thick)}" y="{win_y}" width="{int(wall_thick)}" height="{win_h}" fill="#bfe3f5" stroke="#0e7490"/>'
        )
    ...
    if "двускатн" in str(roof_type).lower() or "gable" in str(roof_type).lower():
        ...
        lines.append(
            f'<polygon points="{x1},{roof_top} {ox},{roof_top - int(ridge_h)} {x2},{roof_top}" class="roof-section"/>'
        )
    elif "плоск" in str(roof_type).lower() or "flat" in str(roof_type).lower():
        lines.append(
            f'<rect x="{x1 - 10}" y="{roof_top - int(0.3 * scale)}" width="{int(width * scale) + 20}" height="{int(0.3 * scale)}" class="roof-section"/>'
        )
    ...
    lines.append(
        f'<text x="{dim_x - 5}" y="{oy - int(total_h * scale / 2)}" class="dim" text-anchor="end">{total_h} м</text>'
    )
...
def generate_elevation_svg(params: dict, side: str = "front") -> str:
    ...
    # Main wall
    lines.append(
        f'<rect x="{x1}" y="{y1}" width="{int(facade_w * scale)}" height="{int(total_h * scale)}" class="wall-facade"/>'
    )
    ...
    for fl in range(floors):
        ...
        for wx_offset in [-facade_w / 3, 0, facade_w / 3]:
            ...
            # Window cross
            lines.append(
                f'<line x1="{wx + int(win_w / 2)}" y1="{wy}" x2="{wx + int(win_w / 2)}" y2="{wy + int(win_h)}" stroke="#0e7490" stroke-width="0.5"/>'
            )
            lines.append(
                f'<line x1="{wx}" y1="{wy + int(win_h / 2)}" x2="{wx + int(win_w)}" y2="{wy + int(win_h / 2)}" stroke="#0e7490" stroke-width="0.5"/>'
            )
    ...
    if "двускатн" in str(roof_type).lower() or "gable" in str(roof_type).lower():
        ...
        lines.append(
            f'<rect x="{x1 - 10}" y="{y1 - int(0.2 * scale)}" width="{int(facade_w * scale) + 20}" height="{int(0.2 * scale)}" class="roof-facade"/>'
        )
...
def generate_mep_diagram_svg(params: dict, mep_calc: dict) -> str:
    ...
    # Building outline
    lines.append(
        f'<rect x="{x1}" y="{y1}" width="{int(width * scale)}" height="{int(length * scale)}" fill="none" stroke="#1a2230" stroke-width="1" stroke-dasharray="4,2"/>'
    )
    ...
    lines.append(
        f'<text x="{sw_x + int(0.3 * scale) + 5}" y="{y1 + 25}" class="label" fill="#78350f">Канализация</text>'
    )
    ...
    lines.append(
        f'<line x1="{panel_x}" y1="{panel_y}" x2="{x2 - int(0.3 * scale)}" y2="{panel_y}" class="electrical"/>'
    )

    # Ventilation (gray, horizontal at top)
    vent_y = y1 + int(0.2 * scale)
    lines.append(
        f'<line x1="{x1 + int(0.5 * scale)}" y1="{vent_y}" x2="{x2 - int(0.5 * scale)}" y2="{vent_y}" class="vent"/>'
    )
    ...
    lines.append(
        f'<line x1="{margin}" y1="{ly + 10}" x2="{margin + 20}" y2="{ly + 10}" class="cold-water"/><text x="{margin + 25}" y="{ly + 13}" class="label">ХВС</text>'
    )
    lines.append(
        f'<line x1="{margin + 60}" y1="{ly + 10}" x2="{margin + 80}" y2="{ly + 10}" class="hot-water"/><text x="{margin + 85}" y="{ly + 13}" class="label">ГВС</text>'
    )
    lines.append(
        f'<line x1="{margin + 120}" y1="{ly + 10}" x2="{margin + 140}" y2="{ly + 10}" class="sewer"/><text x="{margin + 145}" y="{ly + 13}" class="label">Канализация</text>'
    )
    lines.append(
        f'<line x1="{margin + 200}" y1="{ly + 10}" x2="{margin + 220}" y2="{ly + 10}" class="electrical"/><text x="{margin + 225}" y="{ly + 13}" class="label">Электрика</text>'
    )
